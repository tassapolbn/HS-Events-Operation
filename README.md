...
- **By date** shows every session in global date/time order, with its event heading directly above it. There is no extra date toolbar. Past sessions can be folded individually or all at once, then shown again. A session becomes past at its finish time, or at midnight in Bangkok if no finish time is set. Dates remain visible inside session cards, and unassigned tasks stay on their event date.
- **Session floor plans:** open Add/Edit Session, choose an event attachment or upload PNG, JPG, or PDF (20 MB maximum), and Save. Each session can select its own plan. Unassigned event images remain the shared plans. Plans render directly on the board, while the event is expanded; folding an event also hides all its plans; the expand control opens the larger viewer.
- **Inline editing:** signed-in Admin / Events Team can click event, session, or task text directly on the board. Enter saves, Shift+Enter adds a line, and Escape cancels. The right-side pencil still opens the full editor. Edit dialogs also support Enter to Save/OK.
- **Task notifications:** task details → Notify (updated / added task) → choose the change type → Send task notification. Only the saved task is sent to its assigned department. Saving edits never sends a notification automatically. Deploy the updated `send-notification` Edge Function with JWT verification enabled; no additional schema migration or email configuration is required for task notifications.
...
- **Worksheet actions:** right-click a cell, row number, editing text box, or department task card to open relevant actions. Use Actions for touch screens and Shift+F10 for keyboards. Cell clearing is separate from row deletion. Insert above/below keeps the same session and department; bulk add accepts 1–100 rows. Undo/Redo supports cell updates and row creation/deletion.
- **More shortcuts:** Shift+Space selects rows, Ctrl+Space selects a column, Ctrl+Enter applies typed text across a selection, Ctrl+Alt+Up/Down inserts above/below, Ctrl+Shift+D duplicates rows, Ctrl+Alt+Backspace deletes rows, Ctrl+Y or Ctrl+Shift+Z redoes, Ctrl+Home/End jumps to the table ends, Ctrl+F searches tasks, and Alt+Enter opens task details. Shortcuts act only when the worksheet is focused. Native copy/paste and text undo remain available while editing.
