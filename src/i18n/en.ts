export const en = {
  app: {
    name: 'Event Operations',
    school: 'HeadStart International School'
  },
  roles: {
    admin: 'Administrator',
    events_team: 'Events team',
    department_manager: 'Department manager',
    department_staff: 'Department staff'
  },
  users: {
    title: 'Accounts',
    subtitle: 'Who can sign in, what they can do, and closing an account when somebody leaves.',
    add: 'Add someone',
    addHint: 'Give them a username if they have no school mailbox. An email address works too, and either one signs them in.',
    create: 'Create the account',
    created: 'Account created',
    accounts: 'accounts',
    search: 'Search name, username or email',
    none: 'No account matches that.',
    you: 'You',
    closed: 'Closed',
    fullName: 'Full name',
    username: 'Username',
    usernameHint: '3 to 32 letters, digits, dot, dash or underscore',
    emailOptional: 'Email (optional)',
    emailHint: 'Leave empty if they have no school mailbox',
    role: 'Role',
    noDepartment: 'No department',
    passwordHint: 'At least 8 characters. Shown as you type, so you can pass it on correctly.',
    recoveryEmail: 'Send password resets to',
    recoveryHint: 'Leave empty to use the shared events mailbox',
    lastSignIn: 'Last signed in',
    neverSignedIn: 'Never signed in',
    setPassword: 'Set a password',
    passwordSet: 'Password changed',
    sendReset: 'Email a reset link',
    resetSent: 'If that account exists, a reset link has been sent.',
    close: 'Close this account',
    reopen: 'Reopen this account',
    nowClosed: 'Account closed. They can no longer sign in.',
    nowOpen: 'Account reopened.',
    delete: 'Delete for good',
    deleted: 'Account deleted',
    deleteConfirm: 'Delete this account for good?',
    deleteConfirmText: 'The person disappears from the system. Their name stays on the work they already did. Closing the account instead keeps the record whole and still stops them signing in.',
    closeVsDelete: 'Closing an account stops someone signing in and keeps their name on the work they did. Delete only when an account was created by mistake.',
    nameRequired: 'A name is required.',
    idRequired: 'Give them a username or an email address.'
  },
  nav: {
    dashboard: 'Dashboard',
    events: 'Events',
    requests: 'Department Requests',
    calendar: 'Calendar',
    templates: 'Templates',
    myDepartment: 'My Department',
    notifications: 'Notifications',
    users: 'Accounts'
  },
  common: {
    save: 'Save',
    saving: 'Saving...',
    cancel: 'Cancel',
    delete: 'Delete',
    edit: 'Edit',
    create: 'Create',
    close: 'Close',
    search: 'Search',
    filters: 'Filters',
    clearFilters: 'Clear filters',
    loading: 'Loading...',
    noResults: 'No results found',
    confirm: 'Confirm',
    back: 'Back',
    add: 'Add',
    remove: 'Remove',
    view: 'View',
    all: 'All',
    today: 'Today',
    actions: 'Actions',
    name: 'Name',
    status: 'Status',
    priority: 'Priority',
    department: 'Department',
    date: 'Date',
    time: 'Time',
    location: 'Location',
    description: 'Description',
    notes: 'Notes',
    attachments: 'Attachments',
    history: 'History',
    optional: 'optional',
    download: 'Download',
    preview: 'Preview',
    signOut: 'Sign out',
    language: 'Language',
    lightMode: 'Light mode',
    darkMode: 'Dark mode',
    send: 'Send',
    sending: 'Sending...',
    createdBy: 'Created by',
    updatedAt: 'Updated',
    markAllRead: 'Mark all as read',
    viewAll: 'View all',
    previous: 'Previous',
    next: 'Next',
    readOnly: 'Read only',
    unsavedChanges: 'You have unsaved changes.',
    errorGeneric: 'Something went wrong. Please try again.',
    savedSuccess: 'Saved successfully',
    deletedSuccess: 'Deleted successfully',
    overview: 'Overview'
  },
  auth: {
    loginId: 'Username or email',
    loginIdPlaceholder: 'your username',
    forgotPassword: 'Forgotten your password?',
    resetTitle: 'Set a new password',
    resetHint: 'Type your username or email. A reset link is sent to the events mailbox, and whoever looks after it will pass it on.',
    resetSend: 'Send a reset link',
    resetSent: 'If that account exists, a reset link is on its way to the events mailbox. It can be used once and stops working after an hour.',
    resetFailed: 'That link has expired. Ask for a new one from the sign in page.',
    resetDone: 'Password changed. Taking you in now.',
    resetSave: 'Save the new password',
    backToSignIn: 'Back to sign in',
    newPassword: 'New password',
    confirmPassword: 'Type it again',
    passwordRule: 'At least 8 characters',
    passwordTooShort: 'Use at least 8 characters.',
    passwordMismatch: 'The two passwords do not match.',
    signInTitle: 'Welcome back',
    signInSubtitle: 'Sign in to manage school event operations',
    email: 'Email address',
    password: 'Password',
    signIn: 'Sign in',
    signingIn: 'Signing in...',
    invalidCredentials: 'That username or password is not right. Please try again.',
    noAccount: 'Accounts are created by the school administrator.',
    openBoard: 'View the display board',
    openBoardHint: 'No sign in needed. For all department teams.'
  },
  dashboard: {
    title: 'Dashboard',
    welcome: 'Here is what is happening today',
    todaysEvents: "Today's Events",
    upcomingEvents: 'Upcoming Events',
    departmentWorkload: 'Department Workload',
    pendingTasks: 'Pending Tasks',
    completedTasks: 'Completed Tasks',
    recentActivity: 'Recent Activity',
    noEventsToday: 'No events scheduled for today',
    noUpcoming: 'No upcoming events in the next 14 days',
    noActivity: 'No recent activity',
    openTasks: 'open tasks',
    quickCreateEvent: 'New Event',
    quickCreateRequest: 'New Request',
    eventsThisWeek: 'Events this week',
    calendar: 'Calendar'
  },
  events: {
    contacts: 'Who to ask',
    contactsHint: 'The people a department should ask about this event. Leave it empty if there is nobody in particular.',
    contactsEmpty: 'Nobody named yet. Departments will have to ask around.',
    contactAddFirst: 'Name someone to ask',
    contactAdd: 'Add another person',
    contactRemove: 'Remove this person',
    contactNumber: 'Contact',
    contactName: 'Name',
    contactRole: 'Their part in this',
    contactRolePlaceholder: 'Event owner, MC, stage manager',
    contactPhone: 'Phone',
    contactEmail: 'Email',
    contactsOnBoard: 'Ask',
    title: 'Events',
    newEvent: 'New Event',
    editEvent: 'Edit Event',
    deleteEvent: 'Delete event',
    deleteConfirm: 'Delete this event?',
    deleteConfirmText: 'The event and all department tasks will be removed. This cannot be undone.',
    eventName: 'Event Name',
    category: 'Category',
    eventDate: 'Event Date',
    eventLocation: 'Event Location',
    scheduleDateHint: 'Each step can be on a different day. If you leave a date empty, the event date is used.',
    timingNote: 'Or type a timing, e.g. after school',
    headerColor: 'Header Background Color',
    headerTextColor: 'Header Text Color',
    headerColorsHint: 'Used on the event header and the public display board',
    setupStart: 'Setup Start',
    venueReady: 'Venue Ready',
    eventStart: 'Event Start',
    eventFinish: 'Event Finish',
    breakdownStart: 'Breakdown Start',
    breakdownDeadline: 'Breakdown Deadline',
    descriptionLabel: 'Event Description',
    additionalNotes: 'Additional Notes',
    internalNotes: 'Internal Notes',
    internalOnly: 'Visible to the Events Team only',
    timeline: 'Event Timeline',
    departments: 'Departments',
    departmentTasks: 'Department Tasks',
    saveAsTemplate: 'Save as Template',
    notify: 'Send Notification',
    noEvents: 'No events yet. Create your first event to get started.',
    searchPlaceholder: 'Search by event name or location...',
    dateFrom: 'From date',
    dateTo: 'To date',
    assignedStaff: 'Assigned staff',
    eventInfo: 'Event Information',
    schedule: 'Schedule',
    times: 'Times',
    noTasksDept: 'No tasks for this department',
    addTaskFor: 'Add task',
    viewModeOverview: 'Event Overview',
    viewModeDepartment: 'Department View',
    viewModeGrid: 'Grid',
    basicInfo: 'Basic Information',
    contentSection: 'Description & Notes'
  },
  categories: {
    general: 'General',
    academic: 'Academic',
    sports: 'Sports',
    ceremony: 'Ceremony',
    celebration: 'Celebration',
    meeting: 'Meeting',
    performance: 'Performance',
    photoshoot: 'Photoshoot',
    trip: 'School Trip',
    other: 'Other'
  },
  eventStatus: {
    draft: 'Draft',
    scheduled: 'Scheduled',
    active: 'Active',
    completed: 'Completed',
    archived: 'Archived'
  },
  taskStatus: {
    not_started: 'Not Started',
    in_progress: 'In Progress',
    waiting: 'Waiting',
    completed: 'Completed',
    cancelled: 'Cancelled',
    new: 'New',
    acknowledged: 'Acknowledged',
    needs_revision: 'Needs Revision'
  },
  priority: {
    low: 'Low',
    medium: 'Medium',
    high: 'High',
    urgent: 'Urgent'
  },
  tasks: {
    photos: 'Photos',
    photosHint: 'Attach a picture of the thing itself. It is shown with the job on the Display Board and in the email to the department.',
    photosAfterSave: 'Save the job first, then reopen it to attach a photo.',
    addTask: 'Add Task',
    editTask: 'Edit Task',
    taskTitle: 'Task Title',
    taskDescription: 'Task Description',
    instructions: 'Detailed Instructions',
    workLocation: 'Work Location',
    setupLocation: 'Setup Location',
    assignedStaff: 'Assigned Staff',
    startTime: 'Start Time',
    completionTime: 'Completion Time',
    checklist: 'Checklist',
    eventNotesHint: 'Applies to the whole event and is shown on the display board',
    dragHint: 'Drag a task onto another department or session to move it',
    quickAddPlaceholder: 'Type a task and press Enter',
    detailedTask: 'Detailed task (times, instructions, files)',
    addChecklistItem: 'Add checklist item...',
    deleteTask: 'Delete task',
    deleteTaskConfirm: 'Delete this task and its checklist?',
    noTasks: 'No tasks yet',
    taskDetails: 'Task Details',
    departmentNotes: 'Department Notes',
    notesHint: 'Notes from the department team',
    progress: 'Progress',
    copyTask: 'Copy task',
    duplicateTask: 'Duplicate task',
    pasteTasks: 'Paste',
    pasteHere: 'Paste the copied tasks here',
    copiedOne: 'Task copied',
    quickAddPasteHint: 'Paste several lines to add several tasks at once'
  },
  grid: {
    listLayout: 'List',
    boardLayout: 'By department',
    boardHint: 'Click a cell and type. Drag across cells to select them, drag the corner to fill, drag the handle on a job to move it to another department or session.',
    typeToAdd: 'Type to add',
    addSessionBelow: 'Add a session',
    addSession: 'Add session',
    rowToSession: 'Turn this row into a session',
    rowToSessionHint: 'Type the session name in the title cell, the venue in Work Location and the times, then turn the row into a session.',
    sessionCreated: 'Session created',
    rowsMovedIn: 'rows moved into it',
    rowToSessionNeedsTitle: 'Type a name in the title cell first.',
    findReplace: 'Find and replace',
    findWhat: 'Find',
    replaceWith: 'Replace with',
    matchCase: 'Match case',
    onlySelection: 'Only the selected cells',
    replaceAll: 'Replace all',
    replacedCount: 'cells replaced',
    noMatches: 'Nothing matched',
    selectColumn: 'Click to select the whole column',
    selectAllCells: 'Click to select every cell',
    dragRowHint: 'Drag to move this row',
    rowsReordered: 'Rows moved',
    sessionColours: 'Each session has its own colour',
    title: 'Grid',
    addRow: 'Add row',
    duplicateRow: 'Duplicate',
    fillDown: 'Fill down',
    copy: 'Copy',
    copied: 'Copied',
    pasteCopied: 'Paste copied',
    pasted: 'Pasted',
    duplicated: 'Duplicated',
    rowsAdded: 'Rows added',
    undo: 'Undo',
    undone: 'Change undone',
    columns: 'Columns',
    shortcuts: 'Shortcuts',
    tasksWord: 'tasks',
    fillHandle: 'Drag to fill down or across',
    taskMoved: 'Job moved',
    dragToMove: 'Drag to another column or session',
    fillNotAcross: 'Those columns hold different things, so nothing was filled across.',
    openFullForm: 'Open the full form (description, instructions, files)',
    deletedUndoHint: 'Rows deleted. Press Ctrl+Z to bring them back.',
    footerHint: 'Click a cell and type. Enter saves, Tab moves right, Ctrl+D copies the cell above downwards.',
    helpNavigate: 'Arrow keys move the cursor, Shift with arrows selects a block, Tab moves right',
    helpEdit: 'Type to overwrite, Enter saves, Escape cancels, Delete clears the selection',
    helpCopy: 'Ctrl+C and Ctrl+V copy and paste, including to and from Google Sheets',
    helpFill: 'Ctrl+D fills down, or drag the small square at the corner of the selection down or sideways',
    helpPasteRows: 'Pasting more lines than there are rows adds the extra rows for you',
    helpUndo: 'Ctrl+Z undoes the last change'
  },
  sessions: {
    moveUp: 'Move up',
    moveDown: 'Move down',
    reorderHint: 'Order the sessions of one day. A session on an earlier date always stays first.',
    reordered: 'Session order saved',
    hideFromBoard: 'Hide from the display board',
    showOnBoard: 'Show on the display board',
    hiddenOnBoard: 'Hidden from the display board',
    hiddenHint: 'The session and its work stay here, but nobody sees them on the display board. Use this to cancel a session, or to finish editing it before staff see it.',
    nowHidden: 'Session hidden from the display board',
    nowVisible: 'Session back on the display board',
    title: 'Sessions',
    addSession: 'Add Session',
    editSession: 'Edit Session',
    deleteSession: 'Delete session',
    deleteConfirm: 'Delete this session? Its tasks are kept and moved to the whole-event group.',
    sessionTitle: 'Session Name (optional)',
    hint: 'A session is one part of the event: a specific day, venue or time slot',
    generalTasks: 'Whole event tasks',
    whole: 'Whole event',
    sessionNote: 'Session note',
    sessionNoteHint: 'Optional. Shown on the board under this session, for OT, reminders or venue notes.',
    duplicateSession: 'Duplicate session',
    duplicateHint: 'Same details, new date. Change the date or time, then save.',
    copyTasksToo: 'Copy the tasks in this session',
    resetStatus: 'Set the copied tasks back to Not Started',
    copyFromEvent: 'Copy from another event',
    importTitle: 'Copy sessions from another event',
    importHint: 'Pick an event, then choose the sessions to copy into this event',
    searchEvents: 'Search events',
    chooseSessions: 'Sessions to copy',
    noSourceEvents: 'No other event has sessions yet',
    selectAll: 'Select all',
    dateMode: 'Dates',
    dateKeep: 'Keep the original dates',
    dateShift: 'Start the first session on',
    importAction: 'Copy into this event',
    imported: 'Sessions copied',
    duplicated: 'Session duplicated'
  },
  campus: {
    label: 'Campus',
    all: 'All campuses',
    select: 'Select campus'
  },
  displaySize: {
    label: 'Display size',
    small: 'S',
    medium: 'M',
    large: 'L',
    xlarge: 'XL'
  },
  display: {
    overdue: 'past due, still open',
    overdueHint: 'This work was due by now and has not been ticked off. It stays on the board until it is done.',
    scopeActive: 'Active work',
    scopeDone: 'Done',
    scopeHint: 'Finished work moves to Done. Anything still open stays here until it is ticked off, however late it is.',
    activeEmpty: 'Nothing outstanding. Everything is in Done.',
    doneEmpty: 'Nothing has been finished yet.',
    doneRequestsEmpty: 'No request has been finished yet.',
    leftOpen: 'left open',
    leftOpenHint: 'This day has passed with work still open on it.',
    boardTitle: 'Display Board',
    upNext: 'Upcoming Tasks',
    soon: 'Soon',
    tapToView: 'Tap to view the event',
    eventsTitle: 'Events Display',
    requestsTitle: 'Department Requests Display',
    liveBoard: 'Live board, updates automatically',
    allDepartments: 'All Departments',
    updated: 'Updated',
    refresh: 'Refresh',
    noEvents: 'No upcoming events',
    noRequests: 'No open requests',
    completedLabel: 'Completed',
    tapToComplete: 'Tap when done',
    setStatus: 'Update status',
    files: 'Files',
    reference: 'Reference',
    floorPlan: 'Floor plan',
    viewFloorPlan: 'View floor plan',
    due: 'Due',
    openFull: 'Open the full application',
    askQuestion: 'Ask a question',
    askQuestionHint: 'Choose your department and type your question. The events team will see it when they sign in.',
    yourQuestion: 'Your question',
    questionPlaceholder: 'For example: which door should we use to bring the tables in?',
    questionSent: 'Your question has been sent',
    questionSentHint: 'The events team has been notified and will follow up with your department.',
    combinedHint: 'Showing everything for this department: event work and requests together.',
    eventWork: 'Event work',
    deptRequests: 'Department requests',
    noDeptEvents: 'No event tasks for this department right now.',
    noDeptRequests: 'No open requests for this department.',
    editBoard: 'Edit board',
    openFullEditor: 'Open full editor',
    pickCampus: 'Choose a campus board',
    openBoard: 'Open board',
    chooseView: 'Now viewing',
    yourName: 'Your name',
    namePlaceholder: 'Type your name',
    acknowledgedBy: 'Acknowledged by',
    completedBy: 'Completed by',
    viewByDate: 'By date',
    viewByEvent: 'By event',
    dateViewHint: 'Every event in date order, so work that falls between the days of a longer event is not missed.',
    today: 'Today',
    tomorrow: 'Tomorrow',
    pastDay: 'Already past',
    eventsWord: 'events',
    openEvent: 'Open event',
    overlapTitle: 'Other events fall inside these dates',
    overlapHint: 'This event runs across several days. The work below belongs to other events happening in between, so please check those too.',
    viewerZoomIn: 'Zoom in',
    viewerZoomOut: 'Zoom out',
    viewerFit: 'Fit to the screen',
    viewerPrev: 'Previous file',
    viewerNext: 'Next file',
    viewerNewTab: 'Open in a new tab',
    viewerHint: 'Scroll or pinch to zoom, drag to move, double tap to fit',
    viewerCannotPreview: 'This kind of file cannot be shown here. Open it in a new tab instead.',
    viewerFailed: 'The file could not be opened. Please try again.'
  },
  boardStatus: {
    upcoming: 'Upcoming',
    preparing: 'Preparing',
    live: 'Live',
    breakdown: 'Breakdown',
    completed: 'Completed'
  },
  requests: {
    title: 'Department Requests',
    subtitle: 'General job requests not related to an event',
    newRequest: 'New Request',
    editRequest: 'Edit Request',
    requestTitle: 'Request Title',
    reference: 'Reference',
    headResponsible: 'Head Responsible',
    requestDate: 'Posted date',
    dueDate: 'Due Date',
    selectDepartment: 'Select department',
    noRequests: 'No department requests found',
    deleteRequest: 'Delete request',
    deleteConfirm: 'Delete this request?',
    requestDetails: 'Request Details',
    jobDetails: 'Job Details',
    requestedBy: 'Requested by',
    referencePhotos: 'Reference Photos',
    referencePhotosHint: 'Add photos of the item or location, for example broken equipment or furniture to move',
    addPhotos: 'Add photos',
    searchPlaceholder: 'Search by title, reference or location...',
    setupTime: 'Work starts',
    teardownTime: 'Teardown time',
    teardownAfterSetup: 'Teardown must be after setup'
  },
  calendar: {
    title: 'Calendar',
    month: 'Month',
    week: 'Week',
    day: 'Day',
    noEvents: 'No events on this day'
  },
  templates: {
    title: 'Templates',
    subtitle: 'Reusable event setups',
    saveTemplate: 'Save as Template',
    templateName: 'Template Name',
    templateDescription: 'Template Description',
    includeAttachments: 'Include attachments',
    useTemplate: 'Use Template',
    noTemplates: 'No templates saved yet. Open an event and choose "Save as Template".',
    deleteTemplate: 'Delete template',
    deleteConfirm: 'Delete this template?',
    newEventDate: 'Date for the new event',
    createEvent: 'Create Event',
    creating: 'Creating...',
    savedSuccess: 'Template saved',
    tasksIncluded: 'department tasks included',
    sessionsIncluded: 'sessions kept'
  },
  notify: {
    sessionTitle: 'Send new work to the departments',
    newSinceLast: 'job(s) not sent yet',
    nothingNew: 'Every job in this session has already been sent. Tick anything you want to send again.',
    neverSent: 'Not sent yet',
    lastSent: 'Sent',
    noTasks: 'Tick at least one job to send.',
    willSend: 'One email will go to each of',
    sessionBell: 'Send new work to the departments',
    sessionBellCount: 'not sent yet',
    title: 'Send Notification',
    subtitle: 'Choose which departments should be notified by email and in the app.',
    selectAll: 'Select all departments',
    emailsTo: 'Emails will be sent to',
    sendButton: 'Notify Departments',
    successAll: 'Notifications sent successfully',
    successPartial: 'Notifications recorded. Some emails could not be sent:',
    noDepartments: 'Select at least one department',
    inApp: 'In-app notification',
    emailLabel: 'Email'
  },
  notifications: {
    title: 'Notifications',
    empty: 'No notifications yet',
    emailSent: 'Email sent',
    inAppOnly: 'In-app only'
  },
  audit: {
    title: 'Audit History',
    action: 'Action',
    INSERT: 'Created',
    UPDATE: 'Updated',
    DELETE: 'Deleted',
    field: 'Field',
    oldValue: 'Previous value',
    newValue: 'Updated value',
    noHistory: 'No history recorded',
    by: 'by',
    changes: 'changes',
    empty: '(empty)'
  },
  timeline: {
    setupBegins: 'Setup Begins',
    venueReady: 'Venue Ready',
    eventStarts: 'Event Starts',
    eventEnds: 'Event Ends',
    breakdownBegins: 'Breakdown Begins',
    breakdownComplete: 'Breakdown Complete'
  },
  attachments: {
    drop: 'Drop files here or click to browse',
    hint: 'PDF, PNG, JPG, DOCX, XLSX up to 20 MB',
    unsupported: 'This file type is not supported',
    tooLarge: 'File is larger than 20 MB',
    uploadFailed: 'Upload failed. Please try again.',
    deleteConfirm: 'Delete this file?',
    noFiles: 'No attachments',
    previewNotAvailable: 'Preview is shown via Microsoft Office viewer',
    openTab: 'Open in new tab',
    uploading: 'Uploading...'
  },
  myDept: {
    title: 'My Department',
    eventTasks: 'Event Tasks',
    requests: 'Requests',
    noTasks: 'No tasks assigned to your department',
    updateStatus: 'Update status',
    noDepartment: 'Your account is not linked to a department yet. Please contact the administrator.'
  },
  errors: {
    notFound: 'Page not found',
    notFoundText: 'The page you are looking for does not exist or has been moved.',
    goHome: 'Go to Dashboard',
    loadFailed: 'Could not load data. Please refresh the page.'
  },
  validation: {
    required: 'This field is required',
    nameRequired: 'Please enter a name',
    dateRequired: 'Please select a date',
    departmentRequired: 'Please select a department'
  }
};

export type Dict = typeof en;
