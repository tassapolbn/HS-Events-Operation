    ...
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950">
      ...
      <header className="bg-navy-800 text-white dark:bg-navy-950">
        <div className="mx-auto flex max-w-[1800px] flex-wrap items-center gap-x-6 gap-y-3 px-5 py-4 lg:px-8">
          <img src="/logo-landscape-dark.png" alt="HeadStart International School" className="h-12 w-auto object-contain lg:h-14" />
          <div className="min-w-0 flex-1">
            <h1 className="text-xl font-extrabold tracking-tight lg:text-3xl">{title}</h1>
            <p className="text-xs text-white/60 lg:text-sm">{t('display.liveBoard')}</p>
          </div>
          <div className="hidden text-right sm:block">
            <p className="text-2xl font-bold tabular-nums text-gold-400 lg:text-4xl">{clock}</p>
            <p className="text-xs capitalize text-white/70 lg:text-sm">{dateLine}</p>
          </div>
        ...
        <div className="mx-auto flex max-w-[1800px] flex-wrap items-center gap-2 px-5 pb-4 lg:px-8">
          <button
            ...
            className={cn(
              'rounded-full px-4 py-2 text-sm font-bold transition-colors lg:text-base',
              selectedDepartmentId === ''
          ...
          {departments.map((dept) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'flex items-center gap-2 rounded-full px-4 py-2 text-sm font-bold transition-all lg:text-base',
