...
import { ChevronDown, ListPlus, Plus, Clock } from 'lucide-react';
...
export function DepartmentSection({ department, tasks, canEdit, onAddTask, onOpenTask, onQuickAdd }: DepartmentSectionProps) {
            ...
            <ol className="flex-1 divide-y divide-slate-50 dark:divide-slate-800/60">
              {tasks.map((task, index) => (
                <li
                  ...
                  className="flex cursor-pointer items-start gap-2 px-3 py-2 transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/60"
                >
          ...
          {canEdit && (
            <div className="flex items-center gap-1.5 border-t border-slate-100 p-2 dark:border-slate-800">
              ...
              <button
                ...
                className="rounded-lg p-1.5 text-white transition-opacity disabled:opacity-40"
                style={{ backgroundColor: department.color }}
